...
//  Hegg
//
//  Created by Mamdouh El Nakeeb on 8/27/17.
...
class HaggTayehVC: UIViewController, CLLocationManagerDelegate {
    ...
    func initViews(){
        ...
        let contactIconImg = UIImage(named:"markerIcon")?.withRenderingMode(.alwaysTemplate)
        ...
        let sendBtn = UIButton(frame: CGRect(x: margin * 3 / 2, y: mobileV.frame.maxY + 10, width: self.view.frame.width - margin * 3, height: 44))
        
        sendBtn.setTitle("إرسال مكانى", for: .normal)
    ...
    func sendMsg(){
        ...
        if ((nameTF.text?.isEmpty)! || (mobileTF.text?.isEmpty)!) {
